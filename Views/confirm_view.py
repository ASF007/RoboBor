...
class ConfirmView(View):
    ...
    async def confirm_callback(self, interaction: Interaction, button: Button) -> None:
        ...
        await interaction.response.edit_message(
            content=f"{self.bot.emoji.get('confirm')} **{self.success_msg}**", view=self
        )
    ...
    async def on_timeout(self) -> Optional[float]:
        ...
        if self.message:
            await self.message.edit(
                content=f"{self.bot.emoji.get('cancel')} **{self.timeout_msg}**",
                view=self,
            )
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        if interaction.user.id == self.ctx.author.id:
            ...
            await interaction.response.send_message(
                random.choice(self._response), ephemeral=True
            )
            return False
