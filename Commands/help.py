...
class RoboBorHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[Cog], List[Command]]) -> None:
        ...
        prefix = self.context.clean_prefix # return a clean prefix if the bot was mentioned ie @robotop
        bot = self.context.bot
        icon_url =  bot.user.avatar.url if bot.user.avatar else bot.user.default_avatar.url

        # Setting  up our basic embed
        embed = discord.Embed(color = discord.Color.orange())
        embed.description = f"**Type `{prefix}help [command]` for information on a command.**\n\n"
        embed.set_author(name = bot.user.name, icon_url=icon_url)
        embed.set_footer(text =
            f"{bot.version or 'SuS'} ."
        ...
        for cog, commands in mapping.items():
            if cog is not None: # Ignore No Category commands
                if not cog.qualified_name in self.ignored:
                    ...
                    embed.description += f"{cog_emoji} **{cog.qualified_name}** - {cmds}\n\n" 
    ...
    async def send_cog_help(self, cog: Cog) -> None:
        ...
        # Embed
        embed = discord.Embed(
            title = f"{cog_emoji} {cog.qualified_name}", 
            color = discord.Color.orange()
        )
        embed.description = ""
        embed.set_footer(text = f"Type {prefix}help [command] for information on a specific command.")
